...
- The exact primary key / GSI layout for the `Blogs` table (inferred from query code — see `DATA_MODEL.md` for the reasoning). **`Cards`' key schema is now confirmed**: partition key `setName`, sort key `year` (per the site owner, while wiring up the voting feature — see `DATA_MODEL.md`).
