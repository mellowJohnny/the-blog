...
- CORS / auth configuration on each API Gateway route (most CMS write endpoints are called without an `Authorization` header — see `AUTH.md` for the one exception) — still a gap, since the same broad-access credential does **not** extend to `apigateway:GET` (confirmed denied), so route/CORS config itself remains unverified from here.
