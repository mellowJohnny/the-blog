...
- The exact primary key / GSI layout for the `Blogs` and `Cards` DynamoDB tables (inferred from query code — see `DATA_MODEL.md` for the reasoning).
...
- The source of every Lambda behind the API Gateway endpoints in `API_ENDPOINTS.md` — all but `sendAlertHandler` (SMS broadcast) live only in the AWS Console per your note, so their current implementation could have diverged from the prototype versions checked into `Lambda Functions/`.
