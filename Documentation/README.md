...
- The source of every Lambda behind the API Gateway endpoints in `API_ENDPOINTS.md` — all but `sendAlertHandler` (SMS broadcast), `castVoteHandler` (card set voting), `deleteBlogHandler`, and `deleteCardSetHandler` (both added 2026-08-14) live only in the AWS Console per your note, so their current implementation could have diverged from the prototype versions checked into `Lambda Functions/`.
