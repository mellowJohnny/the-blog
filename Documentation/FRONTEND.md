...
sitemap lists batch-level URLs (home, each `tech.html` `blogType`, each
`waxReviews.html` year+category combination, `lockout.html`) rather
than one entry per post — the crawler extracts individual searchable
records from within each page via its own extraction config, so
per-item URLs aren't what it needs discovery-wise.
