...
- **DynamoDB**: at least `Blogs`, `Cards`, `Checklists`, `Subscribers`, `SubscribersTest` tables. See `DATA_MODEL.md`. **Billing mode**: `Checklists` was switched from Provisioned to **On-Demand** on 2026-08-24, after `searchPlayerName`'s full-table `Scan`-per-search design (see `LAMBDA_FUNCTIONS.md`) exhausted its provisioned RCU ceiling under light manual testing — on-demand removes that ceiling entirely (pay per request unit actually consumed, no reserved/idle charge). Other tables' billing modes haven't been confirmed either way.
