...
- **Auth**: Cognito Hosted UI gates page access to everything under `/cms` (`scripts/auth.js`). Note that this only gates the *page*. All three Autobus SMS endpoints, all 12 cardStack create/update/delete/list endpoints (blog posts, card sets — `scripts/cms.js`), and both checklist-upload endpoints (`scripts/checklistUpload.js`) attach a bearer token via `getAuthToken()`, and (confirmed 2026-08-20 for the first 15, checklist endpoints added after) genuinely enforce it server-side too, via a Cognito Authorizer on each of their API Gateway routes — not just a client-side courtesy. Image upload/list (`cmsImageUploader`, `cmsImagePicker`) are the one remaining *gap* — no token sent, no authorizer. `getChecklistBySetName` (public checklist display on `waxReviews.html`) and `searchPlayerName` (public player search, `playerSearch.html`, including its `?audit=1` data-integrity mode) also have no auth, but that's *by design*, not a gap — same public trust level as the other read-only public GETs (`getBlogs`, `getCardSetsByYear`), not a CMS endpoint. `robots.txt` + a `noindex, nofollow` meta tag on every `/cms` page reduce discoverability but don't affect any of the above. See `Documentation/AUTH.md`.
- **Two unrelated tools share `/cms`**: the "cardStack" content CMS (blog posts + card set reviews, TinyMCE-based, plus a separate checklist-PDF-upload page not TinyMCE-based) and the "Autobus Messaging Platform" (Twilio SMS broadcast tool for a cycling club, nothing to do with blog/card content). A third page, `cms/admin.html`, holds a growing set of self-service site-health checks (broken images, checklist-to-review linkage) that run client-side against the site's own public read APIs — no new backend involved. See `Documentation/CMS_GUIDE.md`.
- **`/cms` HTML page conventions**: all 10 pages under `/cms` follow a
  consistent `<head>` element order, script-tag placement (everything
  in `<head>`, nothing after it or at the bottom of `<body>`), and
  markup hygiene (no dead script tags/commented-out markup/unused
  `id`s, no self-closing void elements). Follow these for any new
  `/cms` page or edit to an existing one — see `Documentation/CMS_GUIDE.md`'s
  "HTML conventions for `/cms` pages" section for the specifics. This
  is scoped to `/cms`; the public pages at the repo root have their
  own established, different head structure.
- **Response-shape defensiveness**: frontend code that reads from several of these APIs (especially in `scripts/cmsBlog.js` and `scripts/cmsCardSet.js`) unwraps the response in multiple possible shapes (raw array, `{body: "...json..."}`, `{body: [...]}`, `{Items: [...]}`) in the same function — a sign the underlying Lambdas' contracts have shifted over time. When touching this code, check what shape the *current* Lambda actually returns rather than assuming the frontend's existing unwrap logic is exhaustive or correct.
