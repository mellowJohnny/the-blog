...
The exceptions are `sendAlertHandler/`, `castVoteHandler/`,
`deleteBlogHandler/`, and `deleteCardSetHandler/` (Lambdas whose source
lives in this repo), each with its own `package.json`
(`sendAlertHandler` has a `twilio` dependency; the other three have
none — they only use `@aws-sdk/client-dynamodb`, which ships with the
Lambda Node.js runtime). None of the four is deployed via any command here — per the comment at the top of each `index.mjs`,
...
- **Except `sendAlertHandler/`, `castVoteHandler/`, `deleteBlogHandler/`, and `deleteCardSetHandler/`, no Lambda source is version-controlled** — every other Lambda behind those API Gateway endpoints is edited directly in the AWS Console. The prototype code checked into `Lambda Functions/` predates the current live behavior in several confirmed ways (missing fields, hardcoded test values, mismatched field names) — treat it as historical reference, not as what's actually deployed. Details: `Documentation/LAMBDA_FUNCTIONS.md`.
