...
- **Auth**: Cognito Hosted UI gates page access to everything under `/cms` (`scripts/auth.js`). Note that this only gates the *page*. The three Autobus SMS endpoints attach a bearer token that's also genuinely enforced server-side via a Cognito Authorizer on their API Gateway routes. The 12 cardStack create/update/delete/list endpoints (blog posts, card sets — `scripts/cms.js`) now attach the same bearer token as of 2026-08-16, but **server-side enforcement isn't live yet** — no API Gateway authorizer is attached to those routes, so they still accept unauthenticated requests until that Console-side work is done (see `Documentation/AUTH.md`). Image upload/list are not yet part of this effort at all. `robots.txt` + a `noindex, nofollow` meta tag on every `/cms` page reduce discoverability but don't affect any of the above.
